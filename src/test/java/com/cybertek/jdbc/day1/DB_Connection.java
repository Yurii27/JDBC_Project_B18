...
// CONNECTION -->> STATEMENT --->> RESULTSET
        ...
        //inititally the cursor is at before first location (right before the first row)
        ...
        rs.next(); // currrently we are at the first row
        ...
        // try to move to next row and get second row data as task
        rs.next();
        System.out.println("first column value using index: --> " + rs.getString(1) );
        System.out.println("first column value using column_name: --> " + rs.getString("Region_id") );
        // printing out second column data
